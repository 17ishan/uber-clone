...
const Suggestions = () => {
  return (
    <div className="max-w-full px-4 sm:px-6 lg:px-8 py-8" id="suggestions">
      <div className="mx-auto flex flex-col w-full lg:w-[1142px] items-center">
        {/* Title */}
        <h2 className="text-2xl sm:text-4xl mr-234 font-bold mb-8 text-center sm:text-left">
          Suggestions
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {suggestions.map((item, index) => (
            <div key={index} className="w-full">
              <div className="bg-gray-100 h-[164px] px-3 py-3 rounded-sm shadow flex justify-between">
                {/* Title + Description */}
                <div className="flex max-w-[208px] w-full flex-col justify-between">
                  <div>
                    <h3 className="text-xl md:text-lg  font-medium mb-1">
                      {item.title}
                    </h3>
